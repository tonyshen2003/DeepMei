...
> 记录日期：2026-08-08（最后更新：2026-08-09）
...
## 10. WebView 全屏修饰符跨页复用事故（2026-08-09）

> 背景：首页新增「文创与周边」跳转（raspjam.com）时，直接照抄了「作品播放」
> 的 WebView 布局，引发顶部遮挡、底部白条，并经历了一轮错误诊断。

### 10.1 现象

1. 「文创与周边」打开后**顶部内容被导航栏遮住、底部延伸到标签栏下面被挡**；
2. 把「作品播放」一并“修复”后，作品播放底部出现白条、网页不到屏幕底部；
3. 连续尝试 `contentInsetAdjustmentBehavior = .never`、`obscuredContentInsets = .zero`、
   透明标签栏、全局 `UITabBarAppearance` 后仍不对，用户明确要求「文创不要改」。

### 10.2 根因

1. **布局修饰符被跨页照抄**：`作品播放` 的
   `.ignoresSafeArea()` + `.ignoresSafeArea(edges: .bottom)` 是“网页全屏延伸 +
   标签栏悬浮”的设计；原样复制给不需要全屏的文创页后，
   顶部钻到导航栏下面、底部钻到标签栏下面，产生遮挡。
2. **修文创时一刀切**：顺手把作品播放的修饰符也删了，导致作品播放失去全屏延伸，
   网页底部上方直接露出标签栏白条。
3. **错误诊断放大问题**：把白条误判成 WKWebView / Liquid Glass 的 WebKit inset 问题，
   叠加了一堆与真实原因无关的修复；还错误断言系统默认标签栏是“白色不透明”——
   iOS 26/27 默认就是**液态玻璃透明悬浮**。

### 10.3 定位过程（可复用）

- 发布基线：`~/Library/Developer/Xcode/Archives/2026-08-09/DeepMei 2026-8-9, 07.41.xcarchive`
  （版本 2.7.0 build 3，07:41 归档、08:01 上传 App Store）。
- 用 `git diff` 对比当前工作区与发布版，再用
  `strings <archive>/Products/Applications/DeepMei.app/DeepMei | rg '树莓酱专属企划'`
  确认发布包二进制内容：作品播放/WebView 就是最原始代码，**没有任何 inset 修复**。
- 结论：全部回退到与发布版一致，只保留用户新要求的文创跳转 → 用户确认正常。

### 10.4 经验

1. **布局修饰符不能跨页面照抄**：`ignoresSafeArea` 的语义取决于页面是否真的需要全屏；
   复制前先问“这个页面需不需要延伸到导航栏/标签栏下面”。
2. **一次只改一个页面**：用户说“这个不要改”，就只动另一个；不要在修 A 时顺手改 B。
3. **“之前没问题”先找版本基线**：先确认最近归档/发布的版本（xcarchive / git 提交），
   用 `git diff` 和二进制字符串对比定位差异，再动手，不要从原理猜。
4. **先确认用户看的是哪个页面、哪个构建**：本次大量无效修复源于没有先确认
   “白条”发生在作品播放还是文创、用户跑的是旧构建还是新构建。
5. **系统行为先验证再下结论**：判断“系统默认是什么”前先查文档或实测，
   不要凭旧版本经验断言（iOS 26/27 标签栏默认是液态玻璃）。
6. **最小化改动原则**：最终修复 = 只保留一行功能新增（文创 NavigationLink），
   其余全部与发布版一致；复杂的推断性修复只会引入更多不一致。
